...
class ConsumptionService(IConsumptionService):
  ...
  def insert_consumption(self, data: Dict[str, Any]) -> None:
    ...
    try:
      if not all(
        key in data for key in ['timestamp', 'ven_id', 'resource_id', 'value']
      ):
        raise ValueError('Missing required fields in consumption data')

      self.repository.insert_consumption(data)
      logger.info(f'Successfully inserted consumption record for VEN {data["ven_id"]}')
  ...
  def get_closest_consumption(self):
    pass
