...
class ProxyVTN:
  def __init__(self,
    ...
    self.primary_vtn = OpenADRClient(
      ven_name=f"{ven_name}_primary",
      vtn_url=primary_vtn_url
    )

    self.secondary_vtn = OpenADRClient(
      ven_name=f"{ven_name}_secondary",
      vtn_url=secondary_vtn_url
    )
  ...
  async def _send_event_response(self, event: Dict[str, Any]):
    ...
    """
    vtn_client = (self.primary_vtn if event['source'] == 'primary'
                  else self.secondary_vtn)
