# src/node_dashboard/interface_manager.py
...
class InterfaceManager:
  ...
  def _create_main_content(self) -> None:
    """
    Creates the main content area for the Gradio interface.
    ...
    with gr.Column(elem_id='main-content', scale=4):
      with gr.Blocks(elem_classes='plot-grid'):
        self.plot_components = [
          ...
          for _ in range(len(self.dashboard.configs))
        ]
  ...
  def _setup_update_callbacks(self, state_var: gr.State) -> None:
    ...
    async def update_load_profile():
      await self.dashboard.data_manager.update_load_profile_data()

    async def update_consumption():
      await self.dashboard.data_manager.update_consumption_data()
    ...
    timer_consumption_data.tick(lambda: asyncio.run(async_update_consumption()), [], [])
